...
class VideoGenerator:
    ...
    # def create_countdown_audio(self):
    #     for num in range(3, 0, -1):
    #         tts = gTTS(text=str(num), lang='en')
    #         filename = f'countdown_{num}.mp3'
    #         tts.save(filename)
    #         # Start countdown after title (at 3 seconds)
    #         self.audio_segments.append((filename, 3 + (3 - num)))
    ...
    def generate_video(self, title, player1, player2, final_num1, final_num2,
        ...
        for frame_num in range(self.fps * 10):
        ...
        y_center = (1920 - insert_video.h) // 2

        # Position the overlay video in center
        insert_video = (insert_video
                        .set_position((x_center, y_center))
                        .set_start(2))

        final_video = CompositeVideoClip([base_video, insert_video])
        ...
        all_audio = title_audio + [insert_video.audio.set_start(2)]
